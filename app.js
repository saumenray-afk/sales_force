...
async function loadDistributors() {
    // Use DEMO data - no real business information
    AppState.distributors = DEMO_DISTRIBUTORS.map((dist, index) => {
    ...
    console.log(`Loaded ${AppState.distributors.length} DEMO distributors`);
}

async function loadPOIs() {
    // Use DEMO POI data
    AppState.pois = DEMO_POIS;
    console.log(`Loaded ${AppState.pois.length} DEMO POIs`);
...
function viewDistributor(index) {
    ...
    const achievement = dist.achievement || 0;
    const formHTML = `
        <div style="padding: 20px;">
            <h3 style="margin-bottom: 16px;">${dist.name}</h3>
            <div style="margin-bottom: 12px;"><strong>📍 Location:</strong> ${dist.address || dist.city}</div>
            <div style="margin-bottom: 12px;"><strong>📞 Phone:</strong> ${dist.phone || 'N/A'}</div>
            <div style="margin-bottom: 12px;"><strong>👤 Contact:</strong> ${dist.contact || 'N/A'}</div>
            <div style="margin-bottom: 12px;"><strong>🏪 Retailers:</strong> ${dist.retailers}</div>
            <div style="margin-bottom: 12px;"><strong>💰 Target:</strong> ${formatCurrency(dist.target)}</div>
            <div style="margin-bottom: 12px;"><strong>📊 Sales:</strong> ${formatCurrency(dist.sales)}</div>
            <div style="margin-bottom: 12px;"><strong>🎯 Achievement:</strong> <span style="color: ${achievement >= 80 ? '#28a745' : achievement >= 60 ? '#ffc107' : '#dc3545'}">${achievement.toFixed(1)}%</span></div>
            
            <div style="display: flex; gap: 12px; margin-top: 20px;">
                <button onclick="navigateToLocation(${dist.lat}, ${dist.lng}, '${dist.name.replace(/'/g, "\\'")}')" style="flex: 1; padding: 14px; background: #28a745; color: white; border: none; border-radius: 6px; font-weight: 600;">
                    🧭 Navigate
                </button>
                <button onclick="callLocation('${dist.phone}')" style="flex: 1; padding: 14px; background: #17a2b8; color: white; border: none; border-radius: 6px; font-weight: 600;">
                    📞 Call
                </button>
            </div>
...
function navigateToLocation(lat, lng, name) {
    ...
    if (!navigator.geolocation) {
        showToast('error', 'Not Supported', 'GPS navigation not supported on this device');
        return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
        (position) => {
            ...
            // Try to open in various navigation apps
            ...
            if (isIOS) {
                ...
                navigationURL = `maps://maps.apple.com/?daddr=${destination}&q=${label}`;
                
                // Fallback to Google Maps if Apple Maps not available
                setTimeout(() => {
                    window.location.href = `https://www.google.com/maps/dir/?api=1&origin=${userLat},${userLng}&destination=${destination}&destination_place_id=${label}`;
                }, 500);
            } else if (isAndroid) {
                // Use Google Maps intent for Android
                navigationURL = `geo:${destination}?q=${destination}(${label})`;
                ...
                window.open(`https://www.google.com/maps/dir/?api=1&origin=${userLat},${userLng}&destination=${destination}&travelmode=driving`, '_blank');
                showToast('success', 'Navigation', 'Opening Google Maps...');
                return;
            }
            
            // Try to open native app
            window.location.href = navigationURL;
            showToast('success', 'Navigation', `Navigating to ${name}`);
        },
        (error) => {
            ...
            showToast('info', 'Navigation', 'Opening maps without current location');
        }
    );
}
...
function callLocation(phone) {
    if (!phone || phone === 'N/A') {
        showToast('warning', 'No Phone', 'Phone number not available');
        return;
    }
    
    window.location.href = `tel:${phone}`;
    showToast('info', 'Calling', 'Opening dialer...');
}
